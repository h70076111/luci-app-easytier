...
function index()
	if not nixio.fs.access("/etc/config/easytier") then
        ...
        entry({"admin", "services", "easytier"}, alias("admin", "services", "easytier", "easytier"),_("ET组网"), 46).dependent = true
	entry({"admin", "services", "easytier", "easytier"}, cbi("easytier"),_("ET组网"), 47).leaf = true
